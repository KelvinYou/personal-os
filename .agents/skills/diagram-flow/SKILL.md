...
2. **Source of truth** — does this diagram need a structured model behind it?
   Applies only once a diagram has a second consumer.
...
Verified 2026-08-18. Re-check before relying on these counts.
...
| `repos/ai-stock-analysis` | 1, generated | `pipeline.json` → SVG (`web/lib/pipeline.ts`) + Mermaid (`architecture.md`) | `architecture-check.yml` runs `sync_architecture.py --check` |
...
- **No `\n` inside a quoted node label.** GitHub's Mermaid does not interpret it
  ...
  not by tooling.
- **`<br/>` is the only markup allowed in a label.** portfolio-website renders
  with `securityLevel: "strict"`, which strips HTML from labels — anything richer
  either disappears or breaks the parse.
...
gets edited more than once, or needs to stay in sync with code.
...
reach across a submodule boundary into `personal-os/scripts/`.
