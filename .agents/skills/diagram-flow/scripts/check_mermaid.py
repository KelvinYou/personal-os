#!/usr/bin/env python3
"""Lint Mermaid blocks in a repository's Markdown for render portability.

Deliberately a lint, not a parser. The diagram bugs that actually ship are
usually *syntactically valid* Mermaid that renders wrong on the target — a
literal `\n` in a label, a class with no `classDef`. A `mermaid.parse()` gate
passes both. Catching them needs rule checks, and rule checks need no Node
toolchain, so this stays Python with zero dependencies.

What it does NOT catch: genuine syntax errors. If that becomes a real failure
mode, add a Node-based `mermaid.parse()` step alongside this — don't grow a
parser here.

Copy this file into the repository it guards and wire it into that repository's
own CI. CI cannot reach across a repository or submodule boundary to a checker
that lives elsewhere.

Usage:
    python3 check_mermaid.py [PATH ...]
    python3 check_mermaid.py --ext md,mdx
    python3 check_mermaid.py --root path/to/repo

With no PATH, scans every git-tracked file matching --ext (default `md`) in the
enclosing repository. Submodules are excluded — they own their own diagrams and
their own CI.

MDX note: `--ext md` silently misses every diagram in a `.mdx` file. Pass
`--ext md,mdx` in any repository whose docs are MDX.
...
def _git_root(start: Path) -> Path:
    """Enclosing git repository of `start`, or `start` itself when not in one."""
    try:
        out = subprocess.run(
...
def tracked_docs(root: Path, exts: list[str]) -> list[Path]:
    patterns = [f"*.{e.lstrip('.')}" for e in exts]
    out = subprocess.run(
...
def main(argv: list[str]) -> int:
    ...
    while i < len(argv):
        arg = argv[i]
        if arg == "--ext" and i + 1 < len(argv):
            exts = [e for e in argv[i + 1].split(",") if e]
            i += 2
        elif arg.startswith("--ext="):
            exts = [e for e in arg.split("=", 1)[1].split(",") if e]
            i += 1
    ...
    if paths:
        targets = [Path(a).resolve() for a in paths]
    else:
        targets = tracked_docs(root, exts)
    ...
    # One label carrying `<b>text</b>` matches the tag pattern twice; report once.
    seen: set[tuple] = set()
    deduped: list[Finding] = []
    for f in findings:
        key = (str(f.path), f.line, f.rule, f.message)
        if key in seen:
            continue
        seen.add(key)
        deduped.append(f)
