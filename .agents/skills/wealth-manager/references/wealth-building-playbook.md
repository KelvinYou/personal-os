...
> 用户当前的纯选股做法只是整个方案里的一小块（satellite），缺的是 core 和资金分层。
>
...
用户目前的做法是直接在 moomoo 上挑 MY/US 个股。这等于**只有 satellite，没有 core**——
所有钱都压在高波动、需要主动判断、且大概率跑输大盘的个股上。现代最佳实践（Bogleheads /
...
用户已有 ~RM50k 存款，多数情况下第 1–3 步已覆盖——重点是把第 3 步以上的"过剩现金"
...
25 岁、30+ 年周期、moderate-growth：业界共识是 **85–90% 股票 / 10–15% 防御资产（债/现金/MMF）**。
...
- **Satellite 25–35%**：用户在 moomoo 的 MY + US 个股（按 investment-framework.md 选）
- 规则：satellite 总值不得超过组合 40%；单只 satellite ≤ 组合 15%（呼应 framework 的集中度上限）。
- 自检：若 satellite 连续 3 年跑不赢 core，把更多钱挪回 core。

> 用户已经在做 satellite。给方案时的正确话术不是"别选股了"，而是"先用 VWRA/CSPX 把 core 建到 65%+，
> 你的选股就成了有兜底的 alpha 尝试"。
...
- **Vanguard 研究（1976–2022, MSCI World）**：一次性投入（lump sum）在约 **68%** 的 12 个月周期里
  跑赢分批投入（DCA），因为市场上涨月份多于下跌月份。所以**有一笔闲钱（如 RM50k 过剩现金）时，
  尽快部署 > 慢慢 DCA**——除非估值明显偏高或心理上扛不住。
- **但用户的主资金是月 RM3,000 定量流入**——这本质上就是 DCA，不是 lump-sum 的对立选择，而是
...
- **优先用新增资金再平衡**：用月 RM3,000 优先补"低于目标"的部分，尽量少卖出（省手续费、避免实现盈亏）。
